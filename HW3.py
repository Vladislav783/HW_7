...
# Тогда метод make_order() вернет строку: *****\n*****\n*****.
# Подсказка: подробный список операторов для перегрузки доступен по ссылке0.
...
class Cell:
    ...
    def __add__(self, other):
        return f'Две клетки - соединяются в одну большую: {self.quantity + other.quantity}'

    def __sub__(self, other):
        sub = self.quantity - other.quantity
        return f'Клетка стала меньше, ее размер: {sub} клеточки' if sub > 0 else 'Клетка исчезла :('
